$(function () {
  $("#fhead").submit(function (ev) {
    ...
    $.ajax({
      method: 'post',
      url: `<?= base_url() ?>admin/home/testimoni/update_head`,
      data: form,
      cache: false,
      contentType: false,
      processData: false,
  ...
  function dynamic() {
    ...
    const new_table = table_html.DataTable({
      "ajax": {
        "url": "<?= base_url()?>admin/home/testimoni/ajax_data/",
        "data": null,
        "type": 'POST'
      },
      "processing": true,
  ...
  $("#btn-tambah").click(() => {
    $("#tambahModalTitle").text("Tambah Testimoni");
    $('#id').val('');
    $('#name').val('');
    $('#position').val('');
    $('#foto').val('');
  ...
  $("#fmain").submit(function (ev) {
    ...
    $.ajax({
      method: 'post',
      url: '<?= base_url() ?>admin/home/testimoni/' + ($("#id").val() == "" ? 'insert' : 'update'),
      data: form,
      cache: false,
      contentType: false,
      processData: false,
  ...
  $('#OkCheck').click(() => {
    ...
    $.ajax({
      method: 'post',
      url: '<?= base_url() ?>admin/home/testimoni/delete',
      data: {
        id: id
      }
    }).done((data) => {
...
const view_gambar = (datas) => {
  $("#img-view").attr('src', `<?= base_url() ?>/files/home/testimoni/${datas.dataset.data}`)
}
...
const Ubah = (datas) => {
  ...
  $("#tambahModalTitle").text("Ubah Testimoni");
}
$(function () {
  function dynamic() {
    ...
    const new_table = table_html.DataTable({
      "ajax": {
        "url": "<?= base_url()?>admin/home/excess/ajax_data/",
        "data": null,
        "type": 'POST'
      },
      "processing": true,
  ...
  $("#fmain").submit(function (ev) {
    ...
    $.ajax({
      method: 'post',
      url: '<?= base_url() ?>admin/home/excess/' + ($("#id").val() == "" ? 'insert' : 'update'),
      data: form,
      cache: false,
      contentType: false,
      processData: false,
  ...
  $('#OkCheck').click(() => {
    ...
    $.ajax({
      method: 'post',
      url: '<?= base_url() ?>admin/home/excess/delete',
      data: {
        id: id
      }
    }).done((data) => {
...
const view_gambar = (datas) => {
  $("#img-view").attr('src', `<?= base_url() ?>/files/home/excess/${datas.dataset.data}`)
}

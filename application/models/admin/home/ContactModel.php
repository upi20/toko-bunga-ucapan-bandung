...
class ContactModel extends Render_Model
{
    ...
    public function insert($user_id,  $title, $icon, $column,  $description, $status)
    {
        $data = [
            'title' => $title,
        ...
        // Insert users
        $execute = $this->db->insert('contacts', $data);
        $execute = $this->db->insert_id();
        return $execute;
    }

    public function update($id, $user_id,  $title, $icon, $column,  $description, $status)
    {
        $data = [
            'title' => $title,
            'description' => $description,
        ...
        $execute = $this->db->update('contacts', $data);
        return  $execute;
    }
    ...
    {
        // Delete users
        $exe = $this->db->where('id', $id)->update('contacts', [
            'status' => 3,
            'deleted_by' => $user_id,

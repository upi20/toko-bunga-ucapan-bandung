...
class TestimoniModel extends Render_Model
{
    public function getAllData($draw = null, $show = null, $start = null, $cari = null, $order = null)
    {
        ...
        ");
        $this->db->from("home_testimonials a");
    ...
    public function insert($user_id, $name, $position, $foto, $description, $status)
    {
        $data = [
            'name' => $name,
            'position' => $position,
        ...
        // Insert users
        $execute = $this->db->insert('home_testimonials', $data);
        $execute = $this->db->insert_id();
        return $execute;
    }

    public function update($id, $user_id, $name, $position, $foto, $description, $status)
    {
        $data = [
            'name' => $name,
            'position' => $position,
        ...
        $execute = $this->db->update('home_testimonials', $data);
        return  $execute;
    }
    ...
    {
        // Delete users
        $exe = $this->db->where('id', $id)->update('home_testimonials', [
            'status' => 3,
            'deleted_by' => $user_id,
